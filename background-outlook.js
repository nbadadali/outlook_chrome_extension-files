...
async function handleN8NRequest(data) {
    ...
    if (!response.ok) {
            ...
            if (response.status === 401 || response.status === 402 || response.status === 403) {
                // PRODUCTION: console.log('[Background] JWT token issue (401/402/403), attempting automatic refresh and retry');
...
async function handleOAuthFlow() {
    ...
    try {
        ...
        let startData;
        try {
            const startResponse = await safeRequest('https://connector.saai.dev/webhook/outlook/oauth/start', {
                method: 'GET'
            });
        ...
        const redirectUri = 'https://connector.saai.dev/webhook/outlook/oauth/callback';
        const scopes = [
            'openid',
            'profile',
            'email',
        ...
        debugLog('Launching Outlook OAuth flow with PKCE, redirect URI:', redirectUri);
